<script>
import { RouterLink } from 'vue-router';
  export default{
    data(){
      return{
        text:"Products頁"
      }
    },
    components:{
      RouterLink
    },
    mounted(){
      console.log("#router",this.$route.params.id)
    }
  }
</script>

<template>
  <section class="bg-lightgray"> 
    <!-- 菜單 -->
      <div class="py-5 container">
        <!-- 名稱  -->
        <div class="row p-3">
          <div class="col d-flex justify-content-center">商品圖</div>
          <div class="col d-flex justify-content-center">品名</div>
          <div class="col d-flex justify-content-center">價格</div>
          <div class="col d-flex justify-content-center">數量</div>
          <div class="col d-flex justify-content-center">小計</div>
          <div class="col d-flex justify-content-center">移除</div>
        </div>
        <div class="row">
          <div class="col">
            <div class="products-list-area px-3 bg-white rounded-3">
              <!-- 單一品項 -->
              <div class="row align-items-center py-3">
                <div class="col w-100 overflow-hidden">
                  <div class="position-relative" style="padding-top: 100%;">
                    <img src="/src/assets/img/beef.jpg" class="position-absolute top-0 start-0 w-100 h-100 rounded-3" style="object-fit: cover;" alt="...">
                  </div>
                </div>
                <div class="col d-flex justify-content-center">香煎鮭魚燉蔬菜</div>
                <div class="col d-flex justify-content-center">NT$ 359</div>
                <div class="col d-flex justify-content-center align-items-center">
                  <i class="bi bi-dash-circle-fill text-primary fs-4"></i>
                  <input type="number" value="1" class="input-number mx-2" />
                  <i class="bi bi-plus-circle-fill text-primary fs-4"></i>
                </div>
                <div class="col d-flex justify-content-center">NT$ 359</div>
                <div class="col d-flex justify-content-center">X</div>
              </div>
              <div class="section-line"></div>
              <div class="row align-items-center py-3">
                <div class="col w-100 overflow-hidden">
                  <div class="position-relative" style="padding-top: 100%;">
                    <img src="/src/assets/img/beef.jpg" class="position-absolute top-0 start-0 w-100 h-100 rounded-3" style="object-fit: cover;" alt="...">
                  </div>
                </div>
                <div class="col d-flex justify-content-center">香煎鮭魚燉蔬菜</div>
                <div class="col d-flex justify-content-center">NT$ 359</div>
                <div class="col d-flex justify-content-center">
                  <i class="bi bi-dash-circle-fill text-primary fs-4"></i>
                  <input type="number" value="1" class="input-number mx-2" />
                  <i class="bi bi-plus-circle-fill text-primary fs-4"></i>
                </div>
                <div class="col d-flex justify-content-center">NT$ 359</div>
                <div class="col d-flex justify-content-center">X</div>
              </div>
            </div>
          </div>
        </div>
        <div class="row py-3">
          <div class="text-end">
            <div class="col">
              運費：NT$ 150
            </div>
          </div>
        </div>
        <div class="row pb-4">
          <h4 class="text-end">
            <div class="col">
              總計：NT$ 868
            </div>
          </h4>
        </div>
        <div class="row">
          <div class="col text-end">
            <RouterLink to="/products">繼續逛逛</RouterLink>
            <button type="button" class="btn btn-primary px-5 ms-4">結帳</button>
          </div>
        </div>
      </div>
    </section>
</template>

<style lang="scss" scoped>
  @import "@/assets/all.scss";
  .section-line{
    border-bottom: 2px solid $gray-300;
  }
  .bg-lightgray{
    background: $gray-200;
  }
  .products-list-area{
    box-shadow: 0px 2px 4px 2px lightgray;
  }
  .input-number{
    width: 42px;
  }
</style>